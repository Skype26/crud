        ...
        <div class="content">
            <div class="container-fluid text-center mt-6">
                ...
                <div class="d-flex justify-content-between aling-items-center" style="paddingtop: 30px">
                    <h3 class="mb-0">Productos: Eliminar Datos</h3>
                </div>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btnDelete'])) {
    ...
    if (!empty($nombre) && !empty($descripcion) && !empty($modo) && !empty($categoria) && !empty($subcategoria) && !empty($precio) && !empty($stock)) {
        $sql = "DELETE from productoa where codi_prod=?";
...
?>
    ...
    <?php if ($producto_id > 0): ?>
        ...
        <a href="admin.php" class="btn btn-success">Volver a Productos</a>
    </form>
    <?php else: ?>
        <div class="alert alert-warning">No se ha seleccionado ningún producto válido para borrar.</div>
    <?php endif; ?>
</div>
