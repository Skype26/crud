...
$id_subcategoria =$_GET["id_subcategoria"] ? $_GET["id_subcategoria"] : "";

$stmt = $enlace->prepare("SELECT nom_subcat FROM tb_subcat WHERE id_subcat = ?");
$stmt->bind_param("s", $id_subcategoria);
$stmt->execute();
$stmt->store_result();
        ...
        <div class="h-screen w-full flex flex-col">
            ...
            <div class="w-full">
                ...
                <div class="flex justify-between items-center">
                    <h1 class="p-10 text-5xl font-bold">Editar Sub Categoria</h1>
                </div>
                ...
                <div class="bg-white pt-3">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        if (!empty($nombre)) {
                            $stmt = $enlace->prepare("UPDATE tb_subcat SET nom_subcat=? WHERE id_subcat = ?");
                            $stmt->bind_param("si", $nombre, $id_subcategoria);
                            $stmt->execute();
                            $stmt->close();
                            header("Location: subcategoria_edit.php?id_subcategoria=" . $id_subcategoria . " &success=1");
                            exit;
                        } else {
                            header("Location: subcategoria_edit.php?id_subcategoria=" . $id_subcategoria . " &error=1");
                            exit;
                        }
                    }
